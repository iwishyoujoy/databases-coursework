...
DROP TABLE Item_in_Order CASCADE;
DROP TABLE Procedure_Category CASCADE;
DROP TABLE Favorite_product CASCADE;
...
DROP TABLE Bimbo_Order CASCADE;
...
INSERT INTO Product(id_item, name, description, photo_url, amount_available, price, seller_id, product_category_id) VALUES (0, 'Yves Saint Laurent Baby Doll', 'Exude sophistication with our Yves Saint Laurent Baby Doll. Its elegant design and high-quality materials make it a must-have for any fashion enthusiast.', 'https://avatars.mds.yandex.net/get-mpic/4453979/img_id7874783838998786467.jpeg/600x600', 100, 20000, 1, 7);

INSERT INTO Product(id_item, name, description, photo_url, amount_available, price, seller_id, product_category_id) VALUES (0, 'sparkly lipstick in pinkish color', 'Make a bold statement with our Sparkly Lipstick in Pinkish Color. Its vibrant color and sparkly finish make it a stunning addition to any makeup routine.', 'https://i.pinimg.com/564x/40/6d/6a/406d6a967aa702b6b59fb2a7aee2a25b.jpg', 1000, 700, 1, 5);
...
-- 1 триггер – Ограничение количества продуктов в избранном – 10

CREATE OR REPLACE FUNCTION enforce_favorite_count() RETURNS trigger AS $$
DECLARE
    max_favorite_count INTEGER := 10;
...
BEGIN
    ...
    IF must_check THEN
        ...
        SELECT INTO favorite_count COUNT(*) 
        FROM favorite_product 
        WHERE customer_id= NEW.customer_id;

        IF favorite_count >= max_favorite_count THEN
            RAISE EXCEPTION 'Cannot insert more than % favorite products for each user.', max_favorite_count;
        END IF;
    END IF;
...
CREATE TRIGGER limiting_products_num_in_trigger 
    BEFORE INSERT OR UPDATE ON favorite_product 
    FOR EACH ROW EXECUTE PROCEDURE enforce_favorite_count();
...
-- 2 триггер - Добавление в Item до добавления в Product
...
-- 3 триггер - Добавление в Item до добавления в Appointment
...
-- 4 триггер - Нельзя записаться на одно время на разные процедуры
CREATE OR REPLACE FUNCTION check_appointment_time() RETURNS TRIGGER AS $$
DECLARE
  item_type VARCHAR(32);
  current_appointment TIMESTAMP;
  new_appointment_time TIMESTAMP;
  cur RECORD;
BEGIN
  SELECT type INTO item_type FROM Item WHERE id = NEW.item_id;
  IF item_type = 'appointment' THEN
    SELECT date_time INTO new_appointment_time FROM Appointment WHERE item_id = NEW.item_id;

    FOR cur IN 
      SELECT date_time FROM Item_in_Order 
      JOIN Appointment ON Item_in_Order.item_id = Appointment.item_id
      WHERE order_id = NEW.order_id AND Item_in_Order.item_id != NEW.item_id
    LOOP
      IF ABS(EXTRACT(EPOCH FROM (new_appointment_time - cur.date_time))) <= 1800 THEN
        RAISE EXCEPTION 'Cannot add procedures with less than or equal to 30 min difference';
      END IF;
    END LOOP;
  END IF;
  RETURN NEW;
...
BEFORE INSERT OR UPDATE ON Item_in_Order
FOR EACH ROW
EXECUTE PROCEDURE check_appointment_time();
